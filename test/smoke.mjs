...
// um pequeno poligono em redor de Viana do Castelo (EPSG:4326)
const areaTeste = {
  type: 'Polygon',
  coordinates: [[
    [-8.85, 41.68], [-8.78, 41.68], [-8.78, 41.72], [-8.85, 41.72], [-8.85, 41.68],
  ]],
};
...
async function main() {
  ...
  } catch (e) { ok(false, '/api/health (servidor ligado?)', e.message); return resumo(); }
  ...
  {
    const r = await getJSON('/api/freguesias/160924'); // Monserrate (exemplo)
    ok(r.status === 200 && r.corpo && r.corpo.info, '/api/freguesias/:dtmnfr devolve info+censos+dem');
  }

  {
    const r = await getJSON('/api/freguesias/abc'); // invalido
  ...
  {
    const r = await getJSON('/api/info'); // sem parametros
  ...
  {
    const r = await postJSON('/api/analise/area', {}); // sem geometria
    ok(r.status === 400, '/api/analise/area sem geometria rejeita (400)');
  }

  {
    const r = await postJSON('/api/analise/elegivel', {
      geometry: areaTeste, cota_min: 100, dist_al: 500, usar_al: true,
    });
